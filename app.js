const rawDays=[{date:'2026-06-20',label:'20. 6.',revenue:1080.01,netSales:934.05,orders:10,aov:93.405,spend:221.41,metaRoas:1.38,ctr:1.57,cpm:5.21,clicks:669,impressions:42521,metaPurchases:4,cr:1.4675052410901468,sessions:477,bei:68},{date:'2026-06-21',label:'21. 6.',revenue:202.82,netSales:159.38,orders:6,aov:52.673,spend:272.69,metaRoas:.68,ctr:1.74,cpm:5.08,clicks:935,impressions:53633,metaPurchases:3,cr:.8849557522123894,sessions:565,bei:21},{date:'2026-06-22',label:'22. 6.',revenue:761.06,netSales:625.52,orders:9,aov:69.502,spend:232.13,metaRoas:1.14,ctr:1.66,cpm:5.54,clicks:697,impressions:41893,metaPurchases:6,cr:1.1131725417439704,sessions:539,bei:53},{date:'2026-06-23',label:'23. 6.',revenue:1098.65,netSales:885.67,orders:14,aov:63.262,spend:214.04,metaRoas:5.05,ctr:1.66,cpm:4.69,clicks:755,impressions:45615,metaPurchases:12,cr:1.2939001848428837,sessions:541,bei:87},{date:'2026-06-24',label:'24. 6.',revenue:182.36,netSales:153.25,orders:2,aov:76.625,spend:211.13,metaRoas:.28,ctr:1.39,cpm:3.86,clicks:762,impressions:54676,metaPurchases:1,cr:.19723865877712032,sessions:507,bei:17},{date:'2026-06-25',label:'25. 6.',revenue:935.18,netSales:796.91,orders:12,aov:68.512,spend:211.53,metaRoas:1.36,ctr:1.56,cpm:4.06,clicks:816,impressions:52141,metaPurchases:4,cr:1.4678899082568808,sessions:545,bei:61},{date:'2026-06-26',label:'26. 6.',revenue:213.36,netSales:176.68,orders:2,aov:88.34,spend:39.27,metaRoas:3.66,ctr:1.51,cpm:4.12,clicks:144,impressions:9531,metaPurchases:1,cr:0,sessions:102,bei:65,partial:true}];
const campaigns=[{name:'[DE] - DABA - A+',spend:757.52,roas:.69,ctr:1.52,cpm:6.96},{name:'[T2] - DABA - A+',spend:203.51,roas:2.01,ctr:1.54,cpm:2.61},{name:'[T2] - VIDEO - A+ - ai v1',spend:165.91,roas:4.84,ctr:1.61,cpm:2.33},{name:'[DE] - VIDEO - A+ - ai v1',spend:133.79,roas:.90,ctr:1.83,cpm:9.77},{name:'[DE] - VIDEO - A+ - perf',spend:56.69,roas:1.77,ctr:1.41,cpm:6.68},{name:'[DE] - VIDEO - A+ - organic',spend:52.40,roas:null,ctr:2.13,cpm:6.20},{name:'[IT] - DABA - A+ - RMK',spend:32.38,roas:9.21,ctr:2.02,cpm:2.84},{name:'ASC+ | EU',spend:0,roas:null,ctr:null,cpm:null}];
const products=[{name:'TORNZ - Baggy Denim Embroidered Jorts',gross:393.14,net:204.16,orders:7},{name:'WINGLY - Baggy Cotton Graphic Shorts',gross:292.66,net:175.60,orders:5},{name:'STRIPASO - Baggy Graphic Jeans',gross:230.47,net:130.49,orders:3},{name:'SYMBOLIKA - Oversized Print Zip Up Hoodie',gross:200.50,net:111.15,orders:2},{name:'WESTY - Oversized Graphic Shirt',gross:156.18,net:88.51,orders:2},{name:'FREAKING - Oversized Print T-shirt',gross:134.49,net:50.52,orders:3},{name:'VERTEBRAX - Boxy Fit Print T-shirt',gross:133.76,net:45.59,orders:3},{name:'SPIKY - Oversized Print T-Shirt',gross:128.68,net:80.71,orders:3},{name:'SKICEN - Oversized Graphic Jersey T-Shirt',gross:127.03,net:76.23,orders:3},{name:'STARRY - Baggy Graphic Jorts',gross:126.81,net:76.09,orders:2}];
...
function statusText(k,v){const s=kpiStatus(k,v);if(v==null)return'bez dát';if(k==='cpm')return'sledovať podľa krajiny';if(s==='pos')return'ideálny stav';if(s==='warn')return'v poriadku';return'slabší výsledok'}

const GP=[
 {k:'metaRoas',label:'Meta ROAS',fmt:'num',min:0,max:6,lo:4.0,hi:4.5,goal:'4,0-4,5',limit:'min 3,5+ · scale ~5,0+'},
 {k:'blendedRoas',label:'Blended ROAS',fmt:'num',min:0,max:11,lo:7.5,hi:8.5,goal:'7,5-8,5',limit:'min 6,7+ · <5,5 slabšie'},
 {k:'pno',label:'PNO / Marketing',fmt:'pct',min:0,max:40,lo:12,hi:13.5,goal:'12-13,5 %',limit:'stabilne ~15 %, >18 % riziko'},
 {k:'cpa',label:'CPA',fmt:'eur',min:0,max:50,lo:20,hi:24,goal:'20-24 €',limit:'max ~26 €, >30 € slabšie'},
 {k:'ctr',label:'CTR Meta',fmt:'pct',min:0,max:4,lo:2.2,hi:2.8,goal:'2,2-2,8 %',limit:'min ~1,8 %, <1,5 % slabšie'},
 {k:'cpm',label:'CPM',fmt:'eur',flat:true,limit:'podľa krajiny · vysoké CPM + slabý ROAS je negatívny signál'},
 {k:'bei',label:'Business Efficiency',fmt:'num',min:0,max:140,lo:100,hi:140,goal:'100+',limit:'pod 80 slabší scale stav'}
];
...
function renderToday(){const t=rawDays.find(r=>r.partial)||rawDays[rawDays.length-1];const pno=t.spend/t.revenue*100,blended=t.revenue/t.spend,cpa=t.metaPurchases?t.spend/t.metaPurchases:null;
 const cells=[
  ...
  {l:'Meta ROAS',v:num.format(t.metaRoas),s:[kpiStatus('metaRoas',t.metaRoas),statusText('metaRoas',t.metaRoas)]},
  {l:'CPA',v:money(cpa),s:[kpiStatus('cpa',cpa),statusText('cpa',cpa)]},
  {l:'CR',v:pct(t.cr),s:[t.cr>1?'pos':t.cr>.5?'warn':'neg',t.cr>1?'silné':t.cr>.5?'nižšie':'priebežné']},
  {l:'BEI',v:num.format(t.bei),s:[kpiStatus('bei',t.bei),statusText('bei',t.bei)]},
...
const beiBands={id:'beiBands',beforeDraw(chart){if(chart.canvas.id!=='efficiencyChart')return;const{ctx,chartArea,scales}=chart;if(!chartArea||!scales.y)return;const zones=[{from:100,to:140,color:'rgba(21,128,61,.06)'},{from:80,to:100,color:'rgba(180,83,9,.07)'},{from:0,to:80,color:'rgba(220,38,38,.055)'}];ctx.save();zones.forEach(z=>{const y1=scales.y.getPixelForValue(z.to),y2=scales.y.getPixelForValue(z.from);ctx.fillStyle=z.color;ctx.fillRect(chartArea.left,Math.max(chartArea.top,y1),chartArea.right-chartArea.left,Math.min(chartArea.bottom,y2)-Math.max(chartArea.top,y1));});ctx.restore();}};
...
function renderTables(list){
 ...
 return `<tr><td class="lead">${r.label}${r.partial?' ·':''}</td><td class="n">${money(r.revenue)}</td><td class="n">${r.orders}</td><td class="n">${money(r.aov)}</td><td class="n">${money(r.spend)}</td><td class="n ${pno>18?'neg-c':''}">${pct(pno)}</td><td class="n ${blended<5.5?'neg-c':''}">${num.format(blended)}</td><td class="n ${r.metaRoas<3?'neg-c':''}">${num.format(r.metaRoas)}</td><td class="n ${cpa>30?'neg-c':''}">${money(cpa)}</td><td class="n ${r.ctr<1.8?'neg-c':''}">${pct(r.ctr)}</td><td class="n">${money(r.cpm)}</td><td class="n">${pct(r.cr)}</td><td class="n ${r.bei<80?'neg-c':''}">${num.format(r.bei)}</td></tr>`}).join('');
...
document.querySelectorAll('[data-preset]').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('[data-preset]').forEach(x=>x.classList.remove('active'));b.classList.add('active');const p=b.dataset.preset;if(p==='today'){rangeEls.from.value='2026-06-26';rangeEls.to.value='2026-06-26'}if(p==='3d'){rangeEls.from.value='2026-06-24';rangeEls.to.value='2026-06-26'}if(p==='7d'){rangeEls.from.value='2026-06-20';rangeEls.to.value='2026-06-26'}update()}));
...
document.querySelectorAll('[data-bei]').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('[data-bei]').forEach(x=>x.classList.remove('active'));b.classList.add('active');const p=b.dataset.bei;if(p==='today'){beiEls.from.value='2026-06-26';beiEls.to.value='2026-06-26'}if(p==='3d'){beiEls.from.value='2026-06-24';beiEls.to.value='2026-06-26'}if(p==='7d'){beiEls.from.value='2026-06-20';beiEls.to.value='2026-06-26'}renderBei()}));
...
document.querySelectorAll('[data-gp]').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('[data-gp]').forEach(x=>x.classList.remove('active'));b.classList.add('active');const p=b.dataset.gp;if(p==='today'){gpEls.from.value='2026-06-26';gpEls.to.value='2026-06-26'}if(p==='3d'){gpEls.from.value='2026-06-24';gpEls.to.value='2026-06-26'}if(p==='7d'){gpEls.from.value='2026-06-20';gpEls.to.value='2026-06-26'}renderGuards()}));
